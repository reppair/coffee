...
- [x] **2.2a** Product enhancements — add `category_sort_order` column for per-category ordering using `spatie/eloquent-sortable`, category column in ProductsTable links to CategoryResource view page, `ProductsRelationManager` on CategoryResource view page (not edit) with drag-and-drop reordering, columns (name, type, active), ViewAction only (no create/edit/delete), tests for sortable behavior, relation manager display/actions, and category link URL generation
