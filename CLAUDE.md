...
=== .ai/specs_and_docs rules ===

# Specs and Docs

The specs and docs will be the single source of truth for this project. We use spec-driven development.

## Process
- As we work and make any decisions to change, add and remove functionality, we should keep track of the important decisions in `specs/DECISIONS.md`. Follow the short and concise format of the document.
- Any time we make a change also update any docs related if it affects the current specs.
- Document any new features we come up with as well.
- Update any files in `specs/` as needed based on our current conversation, the goal is to keep any specs and docs about the project up-to-data.

## Changelog

Keep a simple changelog file in the root of the project. It should not bee too technical, rather should focus on features, like the one a user would like to read with each new release / deployment.
